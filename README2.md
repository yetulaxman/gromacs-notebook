...
Make sure to use a different portnumbers by each user to avoid port collision on login nodes.
 ...
 - VNC-VMD port: set some random number for MMMM, e.g., MMMM = 7000 + the number in your training account(Final URL for VNC-VMD will be ine the form:http://localhost:MMMM/vnc.html?password=xxxx; One can copy exact URL  from the terminal when you launched container)
